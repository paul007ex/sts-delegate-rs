...
| R-082 | Real PQC support must be first-class when implemented, not marker-only or fallback-only. | must | implemented | jose/security | issue #19/#29/#72/#73/#74/#78; RFC 9964; OpenSSL 3.5 EVP_PKEY-ML-DSA | `cargo test -p sts-jose --features pqc-openssl-unstable --lib`; `cargo test -p sts-http --features pqc-openssl-unstable`; `cargo test -p sts-cli --features pqc-openssl-unstable exchange_` | Uses OpenSSL 3.5+ ML-DSA through `openssl-rs` behind `pqc-openssl-unstable`; PQC-preferred target policy fails closed unless explicit non-PQC fallback is allowed; no FIPS-validated or stable-backend claim. |
...
| R-130 | Native PQC signing/JWKS/downstream verification is a v2 requirement. | must | implemented | jose/security | repo instructions; RFC 9964; issue #19/#29/#72/#73/#74/#78 | JOSE, HTTP, and CLI feature tests; `sts-cli pqc preflight` | RFC 9964 AKP/ML-DSA is opt-in behind OpenSSL-backed `pqc-openssl-unstable`; target policy can prefer/require PQC with observable fallback evidence; do not claim FIPS-validated PQC, JWE, ML-KEM, KMS/HSM, or PQC rotation. |
...
| JOSE signing and JWKS | `signer.py`, `keys.py` | `sts-jose` | keep RS256 default; opt-in ML-DSA feature; external signer provider boundary | Key custody/JWKS private leak or provider fallback | JOSE tests, JWKS black-box tests, mock-external bootstrap tests | Revert signer backend change |
...
| #19 | jose/pqc/security | Native PQC signing, JWKS publication, and downstream verification are implemented behind `pqc-openssl-unstable` | JOSE/PQC | closed | Feature-gated tests and docs landed; OpenSSL backend drift resolved in #29. |
...
- Native PQC signing/JWKS/downstream verification is implemented only behind the explicit OpenSSL-backed `pqc-openssl-unstable` feature; PQC-preferred target policy now fails closed unless explicit non-PQC fallback is allowed; classical RS256 remains the default (#19/#29/#72/#73/#74/#78).
