...
| R-070 | Nested incoming `act` chains must be preserved when building delegation `act`. | must | implemented | core | RFC 8693 Section 4.1; `crates/sts-core/src/lib.rs:217`; issue #31 | `build_act_nests_prior_chain` | Full incoming nested-chain verification coverage remains tracked in #31. |
...
| #31 | core/tests | Nested incoming `act` chain verification coverage caveat for R-070 needs closure | Delegation act chain | open | Add Python-oracle-backed accepted/rejected-path tests or split the requirement. |
