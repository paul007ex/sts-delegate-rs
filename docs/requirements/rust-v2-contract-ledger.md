...
- Rust issues: #1 through #55, with #2 as the contract-freeze tracker
- Primary specs: RFC 6749, RFC 7523, RFC 7519, RFC 8414, RFC 8693, RFC 9068,
  RFC 9110, RFC 9449, and the active OAuth 2.1 draft for token-endpoint
...
| R-018 | `sts-jose` owns JOSE/JWK/JWKS, signing backend selection, and the external signer provider boundary. | must | implemented | jose | `crates/sts-jose/src/lib.rs:3`; issue #43 | `cargo test -p sts-jose --lib`; `cargo test -p sts-http --lib mock_external` | PQC and unimplemented cloud KMS/HSM providers must fail closed until a concrete backend exists. |
...
| R-020 | `sts-replay` owns replay state, fixed-size replay keys, and the operator-selectable replay-store boundary. | must | implemented | replay | `crates/sts-replay/src/lib.rs:3`; issue #44 | `cargo test -p sts-replay --lib`; shared replay HTTP contract tests | In-memory mode remains local/single-replica only; file-backed mode supports replicas sharing a POSIX directory. |
...
| R-023 | The public HTTP surface is `POST /token`, `POST /introspect`, `POST /revoke`, `GET /jwks`, `GET /.well-known/oauth-authorization-server`, and `GET /.well-known/oauth-protected-resource`. | must | implemented | http | RFC 8693; RFC 7662; RFC 7009; RFC 8414; RFC 9728; issues #68/#88/#112; `crates/sts-http/src/lib.rs` | `contract_discovery_and_jwks_match_python_oracle_shape`; `contract_openapi_json_is_curated_and_docs_are_absent_by_default` | `/exchange` must not return; no `/authorize` claim. |
| R-023B | `/introspect` must require authenticated `application/x-www-form-urlencoded` POST with `token`, accept `token_type_hint`, and return RFC 7662-shaped active/inactive responses for STS-issued access tokens. | must | implemented | http/security | RFC 7662 Sections 2.1/2.2; issue #68; `crates/sts-http/src/lib.rs` | `rfc7662_introspection_active_delegation_token_returns_safe_claims`; `rfc7662_introspection_active_impersonation_token_omits_act`; `rfc7662_introspection_inactive_cases_are_minimal`; `rfc7662_introspection_rejects_missing_or_bad_client_auth`; `rfc7662_introspection_rejects_wrong_content_type` | Inactive responses are exactly `{"active":false}` and do not leak token, raw `jti`, or failure internals. |
| R-023C | `/revoke` must require authenticated `application/x-www-form-urlencoded` POST with `token`, accept `token_type_hint`, and revoke STS-issued access tokens without revealing whether unknown tokens exist. | must | implemented | http/security | RFC 7009 Section 2.1/2.2; issue #88; `crates/sts-http/src/lib.rs` | `rfc7009_revocation_makes_introspection_inactive`; `rfc7009_revocation_unknown_malformed_and_repeated_are_successful`; `rfc7009_revocation_request_validation_and_client_auth` | Revocation stores bounded token fingerprints, not raw tokens or raw `jti`; offline `/jwks` validation cannot observe revocation. |
| R-023D | Protected resource metadata must be served at `GET /.well-known/oauth-protected-resource` with an exact `resource` identifier, truthful `authorization_servers`, enumerable scopes when available, and no invented AS/OIDC claims. | must | implemented | http/docs | RFC 9728 Sections 2.2/3.1; issue #112; `crates/sts-http/src/lib.rs` | `rfc9728_protected_resource_metadata_advertises_target_policy`; `rfc9728_protected_resource_metadata_is_get_only`; `contract_path_bearing_issuer_advertised_endpoints_are_live` | Zero-element metadata arrays are omitted; path-bearing resource aliases remain exact. |
| R-023E | Protected-resource style paths must return RFC 9728/RFC 6750 `WWW-Authenticate: Bearer` challenges with an exact `resource_metadata` URL when bearer tokens are missing, malformed, or invalid; STS/AS endpoints must not emit that challenge. | must | implemented | http/security | RFC 9728; RFC 6750; issue #133; `crates/sts-http/src/lib.rs` | `rfc9728_resource_challenge_covers_missing_malformed_and_invalid_bearer`; `rfc9728_path_bearing_resource_challenge_uses_exact_metadata_url`; `rfc9728_path_bearing_issuer_resource_challenge_uses_exact_metadata_url`; `rfc9728_resource_challenge_is_not_added_to_sts_as_endpoints` | Challenges must not include bearer tokens, raw JWTs, raw `jti`, Authorization header values, or private material. |
| R-024 | Path-bearing issuers must have live path-aware metadata, token, introspection, revocation, protected-resource metadata, and JWKS aliases. | must | implemented | http | RFC 8414 Section 3.1; RFC 9728 Section 3.1; `crates/sts-http/src/lib.rs` | `contract_path_bearing_issuer_advertised_endpoints_are_live` | Root aliases remain live during alpha. |
...
| R-031A | Metadata must not emit `response_types_supported` as an empty array when no authorization endpoint response types are supported. | must | implemented | http | RFC 8414 Section 3.2; issue #52; `crates/sts-http/src/lib.rs:590` | `rfc8414_omits_zero_element_metadata_arrays`; `contract_discovery_and_jwks_match_python_oracle_shape`; `metadata_route_advertises_the_public_contract` | The member is omitted until a real authorization endpoint response type is supported. |
...
| R-035A | Optional `/token` form parameters with empty string values are normalized as omitted after duplicate detection, except required fields still fail required-field validation. | must | implemented | http | OAuth 2.1 draft Section 3.2; RFC 8693 Section 2.1; issues #47/#48/#49/#53 | `rfc_oauth21_empty_grant_type_is_missing_not_unsupported`; `rfc_oauth21_empty_requested_token_type_defaults_to_access_token`; `rfc_oauth21_empty_audience_is_omitted_when_resource_is_present`; `rfc_oauth21_empty_resource_is_omitted_when_audience_is_present`; `rfc_oauth21_empty_client_auth_fields_are_omitted_for_actor_delegation`; `rfc_oauth21_empty_actor_token_fields_keep_both_mode_fail_closed` | `grant_type=` remains invalid; empty `actor_token=` keeps a fail-closed delegation dispatch guard. |
| R-036 | Unknown extension request parameters, including duplicate unknown parameters, must be ignored unless a recognized gate fails. | policy | implemented | http/core | Python `tests/test_integration.py:206`; issues #17/#129 | `contract_unknown_extension_params_are_ignored`; `rfc_oauth21_duplicate_unknown_parameters_are_ignored` | Fresh actor assertion required when testing around replay; duplicate recognized parameters still fail under R-035. |
...
| R-038 | `/token` responses and errors, including method-not-allowed responses, must include `Cache-Control: no-store` and `Pragma: no-cache`. | must | implemented | http | RFC 6749 Section 5.1; Python `tests/test_integration.py:550`; issue #130 | token/error contract tests; `token_method_not_allowed_is_hardened` | Metadata/JWKS are public-cacheable; `/token` 405 keeps `Allow: POST`. |
...
| R-050 | `requested_token_type` absent or empty means the STS mints its default access_token type. | policy | implemented | core/http | RFC 8693 Section 2.1; issues #11/#47 | requested-token contract test; `rfc_oauth21_empty_requested_token_type_defaults_to_access_token` | Default issued_token_type is access_token. |
...
| R-054 | Exactly one target must be resolved from non-empty `audience` and/or `resource`. | policy | implemented | core/http | RFC 8693 Section 2.1.1; issues #48/#53; `crates/sts-core/src/lib.rs:155` | core tests; `rfc_oauth21_empty_audience_is_omitted_when_resource_is_present`; `rfc_oauth21_empty_resource_is_omitted_when_audience_is_present` | Both present must agree; empty `audience=` does not block a valid `resource`. |
...
| R-066 | Minted access tokens use ML-DSA-65 by default; RS256 is explicit compatibility mode only. | must | implemented | jose | `crates/sts-jose/src/lib.rs`; issues #5/#19/#29/#72/#131 | JOSE tests, HTTP contract tests, live PQC canary | PQC selection never silently falls back to RS256. |
...
| R-070 | Nested incoming `act` chains must be preserved when building delegation `act`. | must | implemented | core/http | RFC 8693 Section 4.1; Python `tests/test_stress.py:147`; `crates/sts-core/src/lib.rs:217`; issue #31 | `build_act_nests_prior_chain`; `contract_delegation_preserves_sanitized_nested_prior_act_chain`; `contract_malformed_prior_act_does_not_burn_actor_replay`; `contract_prior_act_chain_rejects_missing_sub_and_excessive_depth` | Prior chains are preserved as nested identity-only `act` members, malformed/deep chains reject, and rejected prior-act gates do not burn actor replay state. |
...
| R-103 | Client assertion `jti` must be recorded only after late gates pass. | must | implemented | replay/http | Python `application/exchange.py:87`; Python `tests/test_integration.py:1523` | `contract_client_assertion_jti_is_not_burned_by_late_target_failure` | Bad target retry succeeds; replay fails only after success. |
...
| R-126 | Release validation requires fmt, workspace tests, clippy, architecture guard, oracle smoke, all-features PQC gates, and diff check. | policy | implemented | release | issue #2/#75 comments; `.github/workflows/ci.yml`; `README.md` | CI all-features tests/clippy; alpha validation | Supply-chain helper CLIs remain optional until installed. |
...
| R-128 | Full Authorization Server features such as authorization endpoint, login, consent, ID tokens, UserInfo, dynamic registration, refresh-token families, logout, and browser sessions are non-goals for current STS alpha. | non-goal | implemented | PM/http | Python docs; `README.md:3`; issues #68/#88/#112 | absence of `/authorize` and OIDC routes; online validation contract tests | `/introspect`, `/revoke`, and protected-resource metadata are now STS online-validation surfaces, not full-AS scope. |
...
| R-130 | Native PQC signing/JWKS/downstream verification is a v2 requirement. | must | implemented | jose/security | repo instructions; RFC 9964; issue #19/#29/#72/#73/#74/#78/#131 | JOSE, HTTP, and CLI tests; `sts-cli pqc preflight` | RFC 9964 AKP/ML-DSA is the default OpenSSL-backed signing path; target policy can require PQC with observable fallback evidence; do not claim FIPS-validated PQC, JWE, ML-KEM, KMS/HSM, or PQC rotation. |
...
| JOSE signing, public JWKS, backend selection, and PQC | `signer.py`; `keys.py`; Python PQC intent docs | JOSE tests, JWKS tests, OpenSSL ML-DSA feature tests, fail-closed selector tests, and docs/README no-FIPS claim | R-079 through R-082, R-130 | covered; PQC ML-DSA-65 is the default signing runtime, with no FIPS claim |
...
| UC-07A | Resource server | Check current online token status | STS-issued token and client assertion | POST `/introspect` with `token` | `token_type_hint` may be present or wrong | Expired/revoked/malformed/wrong-issuer/bad-signature tokens return minimal inactive | Active token response carries safe claims including `act` for delegation | R-023B |
| UC-07B | Resource server or operator | Revoke an STS-issued access token | STS-issued token and client assertion | POST `/revoke` with `token` | Unknown/malformed/repeated revocation still returns 200 | Missing auth or token is rejected | Later introspection returns inactive | R-023C |
| UC-07C | MCP/resource server | Discover protected-resource metadata | Protected resource base URL | GET `/.well-known/oauth-protected-resource` | Path-bearing resource alias returns exact resource value | POST is 405; no false AS/OIDC claims | Client can locate STS issuer and supported scopes | R-023D |
...
| N-03A | Non-POST `/token` method | `405 Method Not Allowed` with no-store hardening and `Allow: POST` | Rust HTTP contract | R-038 |
...
| N-26 | Unauthenticated `/introspect` | `invalid_client`, 401 | Rust HTTP contract; RFC 7662 Section 2.1 | R-023B |
| N-27 | Revoked token introspected online | `{"active":false}` only | Rust HTTP contract; RFC 7662 active-state definition; RFC 7009 revocation | R-023B, R-023C |
| N-28 | Unknown token submitted to `/revoke` | HTTP 200 without existence disclosure | Rust HTTP contract; RFC 7009 Section 2.2 | R-023C |
| N-29 | POST to protected-resource metadata | method-not-allowed | Rust HTTP contract; RFC 9728 Section 3.1 | R-023D |
| N-30 | Unexpected internal error leaks detail | must not leak; clean server_error | Python integration | R-040 |
| N-31 | `/exchange` route exists | must stay 404 | Python integration | R-041 |
| N-32 | Transport dependency added to lower crate | architecture guard failure | issue #3 | R-011 |
| N-33 | Unsafe code added silently | architecture/security failure | workspace lints | R-007 |
| N-34 | Full OAuth AS capability claimed in alpha | must not claim | README release shape | R-128 |
| N-35 | STS issuer contains query, fragment, or non-loopback HTTP scheme | reject config load | issue #13; Python stress tests | R-132 |
...
| E-03 | `response_types_supported` has no values | omit the metadata member instead of emitting a zero-element array | RFC 8414 Section 3.2; Rust HTTP metadata; issue #52 | R-031A |
| E-04 | Unknown extension form param, including duplicate unknown keys | ignore | Python integration; Rust HTTP contract | R-036 |
...
| JOSE signing and JWKS | `signer.py`, `keys.py` | `sts-jose` | keep wire behavior; default ML-DSA-65; explicit RS256 compatibility; external signer provider boundary | Key custody/JWKS private leak or provider fallback | JOSE tests, JWKS black-box tests, mock-external bootstrap tests | Revert signer backend change |
| PQC backend | preview/planned Python work | `sts-jose` feature gate | default ML-DSA-65, no fallback | Claiming FIPS PQC from unstable API | native sign/verify/JWKS/downstream verification tests | Disable PQC feature only through explicit compatibility build/config |
...
| Replay storage | `replay.py`, `application/replay_records.py` | `sts-replay` | keep semantics; store backend can change | late jti preburn, multi-worker | replay tests plus oracle late-failure tests plus shared file-backed replay tests | In-memory default plus file-backed shared replay mode |
...
| #2 | roadmap | Requirements ledger and use-case count not canonical | Contract freeze | closed | Ledger is canonical for current Rust v2 tracking; keep issue-state rows in sync with GitHub. |
...
| #19 | jose/pqc/security | Native PQC signing, JWKS publication, and downstream verification are implemented through the OpenSSL-backed ML-DSA path | JOSE/PQC | closed | Feature-gated tests and docs landed; OpenSSL backend drift resolved in #29; default flip tracked in #131. |
...
| #50 | dpop/rfc | DPoP accepted `htm=post` for an actual `POST` request | DPoP proof validation | fixed on main | Keep lowercase-method rejection test in release gate. |
| #51 | docs/rfc | Ledger overclaimed stale empty-value, metadata, and DPoP behavior | Requirements ledger | fixed on main | Keep ledger rows tied to non-ignored tests and issue IDs. |
| #52 | metadata/rfc | Metadata emitted `response_types_supported: []` even though no authorization endpoint response types are supported | Authorization server metadata | fixed on main | Keep metadata contract expecting omission. |
...
| #54 | dpop/rfc | DPoP `htu` comparison treated `/token/` as equivalent to `/token` | DPoP proof validation | fixed on main | Keep trailing-slash mismatch test in release gate. |
